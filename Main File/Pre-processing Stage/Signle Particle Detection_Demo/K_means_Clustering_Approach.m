...
D = dir('*.tif');
    for n = 1:numel(D)
        ...
        time=toc;
        k_means_time(n)=toc;
        fprintf('K-Means consumed ( %f ) second to cluster the CryoEM image \n',k_means_time(n));
        subplot(3,3,8);imshow(cluster);title('CryoEM Clustred Image')
        % Post Processing
        k=imfill(cluster,'holes');
        BW = imopen(k,SE);imshow(BW,[])
        Preprocessed_cluster=bwareaopen(BW,100);
        subplot(3,3,9); imshow(Preprocessed_cluster);title('Binary Mask Cleaning Image')
        %
        %% Save the cluster MRI Images ...
        imwrite(Preprocessed_cluster, [clust_output_dir D(n).name],'tif');         
        cd (CRYOEM_images_dir);
%         pause;
        close all;
    end
